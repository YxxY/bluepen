...
export default {
  ...
  data() {
    const uuid = new Array(6)
      .map(e => Math.floor(Math.random() * Math.floor(10)))
      .join("");
    return {
      now: 0,
      date: null,
      ...
      diff: 0,
      show: true,
      timeData: Object.keys(this.labels).map(e=>{
        return {
          current: 0,
          previous: 0,
          label: this.labels[e],
          elementId: `flip-card-${e}-${uuid}`
        }
      })
    };
  },
  created() {
    ...
    const startTime = this.deadline;
    this.date = Math.trunc(Date.parse(startTime.replace(/-/g, "/")) / 1000);
    if (!this.date) {
